...
class TestDabbler:
    ...
    def experiment_with_custom_soil(self, test_field):
        experiment = Experiment(
            ...
            coordinates_longitude=-28.3689,
            weather_station_code="UFGA",
            soil_data=dabbler.soil.SoilGenerator().build_soils(
                {"TEST000001": test_field}
            )["TEST000001"],
        )
        return experiment
    ...
    def test_out_fifos_have_been_created(self, dssat_instance):
        fifos_created = [x.name for x in dssat_instance.in_out_location.glob("*.OUT")]
    ...
    def test_deploy_write_thread_for_weather(self, dssat_instance, experiment):
        ...
        test_thread = Thread(
            target=dssat_instance.deploy_write_threads,
        ...
        with open(dssat_instance.in_files["WTH"], "r") as wth:
            output = wth.read()
    ...
    def test_deploy_write_thread_for_soil(self, dssat_instance, experiment):
        ...
        with open(dssat_instance.in_files["SOIL"], "r") as soil:
            output = soil.read()

        # Remove soil file so other DSSAT runs don't look inside it for soil info
        dssat_instance.in_files["SOIL"].unlink()
    ...
    def test_run_with_experiment_with_custom_soil_data(
        self, dssat_instance, experiment_with_custom_soil
    ):
